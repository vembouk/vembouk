...
class Vehicle1 extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('vehicle_model1');
    ...
    public function Add_leaves_Type()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    $success = $this->vehicle_model1->Add_leave_Info($data);
                    echo "Successfully Added";
                } else {
                    $success = $this->vehicle_model1->Update_leave_Info($id, $data);
                    echo "Successfully Updated";
                }
    ...
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['employee']    = $this->vehicle_model1->emselect(); // gets active employee details
            $data['leavetypes']  = $this->vehicle_model1->GetleavetypeInfo();
            $data['application'] = $this->vehicle_model1->AllLeaveAPPlication();
    ...
    public function EmApplication()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['employee']    = $this->vehicle_model1->emselectByID($emid);
            $data['leavetypes']  = $this->vehicle_model1->GetleavetypeInfo();
            $data['application'] = $this->vehicle_model1->GetallApplication($emid);
        ...
        if ($this->session->userdata('user_login_access') != False) {
           ...
           // $first_name1  = $this->input->post('first_name1');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $success = $this->vehicle_model1->Application_Apply_Update($id, $data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                ...
                if ($this->db->affected_rows()) {
                    ...
                    $success = $this->vehicle_model1->Application_Apply_Approve($data);
    ...
    public function Add_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    $success = $this->vehicle_model1->Application_Apply($data);
                    #$this->session->set_flashdata('feedback','Successfully Updated');
                    #redirect("leave/Application");
                    echo "Successfully Added";
                } else {
    ...
    public function Add_L_Status()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $success = $this->vehicle_model1->Application_Apply_Update($id, $data);
            if ($value == 'Approve') {
                $totalday = $this->vehicle_model1->GetTotalDay($type);
                $total    = $totalday->total_day + $duration;
                $data     = array();
                $data     = array(
                    'total_day' => $total
                );
                $success  = $this->vehicle_model1->Assign_Duration_Update($type, $data);
                echo "Successfully Updated";
            } else {
                echo "Successfully Updated";
            }
    ...
    public function LeaveAppbyid()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['leaveapplyvalue'] = $this->vehicle_model1->GetLeaveApply($id);
    ...
    public function LeaveTypebYID()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id                     = $this->input->get('id');
            $data['leavetypevalue'] = $this->vehicle_model1->GetLeaveType($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function GetEarneBalanceByEmCode()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id                     = $this->input->get('id');
            $data['earnval'] = $this->vehicle_model1->GetEarneBalanceByEmCode($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function APPvalueDelet()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id      = $this->input->get('id');
            $success = $this->vehicle_model1->DeletApply($id);
            redirect('vehicle/Application');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function LeavetypeDelet()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id      = $this->input->get('D');
            $success = $this->vehicle_model1->DeletType($id);
            redirect('vehicle1/leavetypes1');
        } else {
            redirect(base_url(), 'refresh');
        }
    }

    public function leavetypes1()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['leavetypes'] = $this->vehicle_model1->GetleavetypeInfo();
            $this->load->view('backend/leavetypes1', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function LeaveType()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id          = $this->input->get('id');
            $year        = date('Y');
            $leavetype   = $this->vehicle_model1->GetemLeaveType($id, $year);
            $assignleave = $this->vehicle_model1->GetemassignLeaveType($id, $year);
    ...
    public function EmLeavesheet()
    {
        $emid              = $this->session->userdata('user_login_id');
        $data              = array();
        $data['embalance'] = $this->vehicle_model1->EmLeavesheet($emid);
    ...
    public function GetemployeeGmLeave()
    {
        ...
        $emid        = $this->input->get('emid');
        $assignleave = $this->vehicle_model1->GetemassignLeaveType($emid, $id, $year);
        $totaldays   = 0;
        foreach ($assignleave as $value) {
            $totaldays = $totaldays + $value->day;
        }
        $day        = $totaldays;
        $leavetypes = $this->vehicle_model1->GetleavetypeInfoid($id);
    ...
    public function Get_LeaveDetails()
    {
        ...
        $report = $this->vehicle_model1->GetEmLEaveReport($emid, $day, $year);
    ...
    public function approveLeaveStatus() {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $success = $this->vehicle_model1->updateAplicationAsResolved($id, $data);
            if ($value == 'Approve') {
                $determineIfNew = $this->vehicle_model1->determineIfNewLeaveAssign($employeeId, $type);
                //How much taken
                $totalHour = $this->vehicle_model1->getLeaveTypeTotal($employeeId, $type);
            ...
            $success  = $this->vehicle_model1->updateLeaveAssignedInfo($employeeId, $type, $data);
            $earnval = $this->vehicle_model1->emEarnselectByLeave($employeeId); 
              $data = array();
              $data = array(
                        'present_date' => $earnval->present_date - ($duration/8),
                        'hour' => $earnval->hour - $duration
                    );
            $success = $this->vehicle_model1->UpdteEarnValue($employeeId,$data);                     
            echo "Updated successfully";
                ...
                //If not taken yet
                    ...
                    $success  = $this->vehicle_model1->insertLeaveAssignedInfo($data);
                    echo "Updated successfully";
                }
            } else {
                echo "Updated successfully";
    ...
    public function LeaveAssign()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if (!empty($leaveID)) {
                $year        = date('Y');
                $daysTaken = $this->vehicle_model1->GetemassignLeaveType($employeeID, $leaveID, $year);
                //die($daysTaken->hour);
                $leavetypes = $this->vehicle_model1->GetleavetypeInfoid($leaveID);
                ...
                if($leaveID =='5'){
                $earnTaken = $this->vehicle_model1->emEarnselectByLeave($employeeID);
    ...
    public function Update_Earn_Leave_Only(){
        ...
        $success = $this->vehicle_model1->UpdteEarnValue($emid,$data);
        echo "Successfully Updated.";
    }
}
