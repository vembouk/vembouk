...
class vendor extends CI_Controller {
    ...
    public function Vendors(){
        ...
        $data['vendor'] = $this->vendor_model->emselect();
    ...
    public function Add_vendor(){
        if($this->session->userdata('user_login_access') != False) { 
        $this->load->view('backend/add-vendor');
        }
    else{
		redirect(base_url() , 'refresh');
	}            
	...
	$blood = $this->input->post('blood');		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->vendor_model->Does_email_exists($email) && $password != $confirm){
            ...
            if (!$this->upload->do_upload('image_url')) {
                ...
                if($id){
            $success = $this->vendor_model->Update($data,$id); 
	...
	$status = $this->input->post('status');		
        ...
        if ($this->form_validation->run() == FALSE) {
            echo validation_errors();
			#redirect("vendor/view?I=" .base64_encode($eid));
			} else {
            if($_FILES['image_url']['name']){
            $file_name = $_FILES['image_url']['name'];
			$fileSize = $_FILES["image_url"]["size"]/1024;
            ...
            $employee = $this->vendor_model->GetBasic($id);
    ...
    public function view(){
        ...
        $data['basic'] = $this->vendor_model->GetBasic($id);
        $data['permanent'] = $this->vendor_model->GetperAddress($id);
        $data['present'] = $this->vendor_model->GetpreAddress($id);
        $data['education'] = $this->vendor_model->GetEducation($id);
        //$data['experience'] = $this->vendor_model->GetExperience($id);
        $data['bankinfo'] = $this->vendor_model->GetBankInfo($id);
        $data['tradeinfo'] = $this->vendor_model->GetTradeInfo($id);
        $data['auditinfo'] = $this->vendor_model->GetAuditInfo($id);
        $data['fileinfo'] = $this->vendor_model->GetFileInfo($id);
        $data['typevalue'] = $this->payroll_model->GetsalaryType();
        $data['leavetypes'] = $this->leave_model->GetleavetypeInfo();    
        $data['salaryvalue'] = $this->vendor_model->GetsalaryValue($id);
        $data['socialmedia'] = $this->vendor_model->GetSocialValue($id);
            $year = date('Y');
        $data['Leaveinfo'] = $this->vendor_model->GetLeaveiNfo($id,$year);
        $this->load->view('backend/vendor_view',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}         
    ...
    public function view1(){
        ...
        $this->load->view('backend/print_view1',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}         
    ...
    public function Parmanent_Address(){
        ...
        if ($this->form_validation->run() == FALSE) {
            echo validation_errors();
			#redirect("vendor/view?I=" .base64_encode($em_id));
			} else {
            $data = array();
                $data = array(
                    'emp_id' => $em_id,
            ...
            if(!empty($id)){
                $success = $this->vendor_model->UpdateParmanent_Address($id,$data);
                $this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";                
            } else {
                $success = $this->vendor_model->AddParmanent_Address($data);
    ...
    public function Add_Education(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_education($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_Education($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";
            }
    ...
    public function Save_Social(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Insert_Media($data);
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_Media($id,$data);
                echo "Successfully Updated";
            }
    ...
    public function Add_Experience(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_Experience($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                echo "Successfully Updated";
            } else {
                $success = $this->vendor_model->Update_Experience($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";
            }
    ...
    public function Disciplinary(){
        if($this->session->userdata('user_login_access') != False) {
        $data['desciplinary'] = $this->vendor_model->desciplinaryfetch();
    ...
    public function add_Desciplinary(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_Desciplinary($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                #redirect('vendor/Disciplinary');
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_Desciplinary($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_bank_info(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_BankInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_BankInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_trade_info(){
     ...
     //$this->form_validation->set_rules('branch_name', 'branch name', 'trim|required|min_length[5]|max_length[120]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_TradeInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_TradeInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_audit_info(){
     ...
     //$this->form_validation->set_rules('branch_name', 'branch name', 'trim|required|min_length[5]|max_length[120]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->vendor_model->Add_AuditInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->vendor_model->Update_AuditInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Reset_Password_Hr(){
        ...
        $success = $this->vendor_model->Reset_Password($id,$data);
        #$this->session->set_flashdata('feedback','Successfully Updated');
        #redirect("vendor/view?I=" .base64_encode($id));
                echo "Successfully Updated";
            } else {
    ...
    public function Reset_Password(){
        ...
        $onep = $this->input->post('new1');
        $twop = $this->input->post('new2');
        $pass = $this->employee_model->GetvendorId($id);
    ...
    public function Department(){
        if($this->session->userdata('user_login_access') != False) {
        $data['department'] = $this->vendor_model->depselect();
    ...
    public function Save_dep(){
       ...
       if ($this->form_validation->run() == FALSE) {
           echo validation_errors();
           redirect('vendor/Department');
       }else{
        $data = array();
        $data = array('dep_name' => $dep);
        $success = $this->vendor_model->Add_Department($data);
        #$this->session->set_flashdata('feedback','Successfully Added');
        #redirect('vendor/Department');
       }
        }
    ...
    $exp_date = $this->input->post('exp_date');  		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $success = $this->vendor_model->File_Upload($data); 
            #$this->session->set_flashdata('feedback','Successfully Updated');
            #redirect("vendor/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
			}
    ...
    public function Inactive_vendor(){
        $data['invalidem'] = $this->vendor_model->getInvalidUser();
        $this->load->view('backend/invalid_vendor',$data);
    }
}