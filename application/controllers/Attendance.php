...
class Attendance extends CI_Controller
{
    ...
    public function Get_attendance_data_for_report()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $attendance_hours1 = $this->attendance_model->getTotalAttendanceDataByID1($employee_PIN, $date_from, $date_to);
            if(!empty($attendance_data)){
            $data['name'] = $attendance_data[0]->name;
            $data['days'] = count($attendance_data);
            $data['hours'] = $attendance_hours;   
            $data['hours1'] = $attendance_hours1;               
            }
    ...
    public function Add_Attendance()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $holiday_ot = $this->input->post('holiday_ot');
