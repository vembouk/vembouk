...
class Payment extends CI_Controller
{
    ...
    public function Holidays()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['holidays'] = $this->payment_model->GetAllHoliInfo();
    ...
    {
        if ($this->session->userdata('user_login_access') != False) {
            $result = $this->payment_model->GetAllHoliInfoForCalendar();
            print_r($result);
            die();
    ...
    public function Add_Holidays()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    $success = $this->payment_model->Add_HolidayInfo($data);
                    $this->session->set_flashdata('feedback', 'Successfully Added');
                    #redirect("leave/Holidays");
                    echo "Successfully Added";
                } else {
                    $success = $this->payment_model->Update_HolidayInfo($id, $data);
    ...
    public function Add_leaves_Type()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    $success = $this->payment_model->Add_leave_Info($data);
                    echo "Successfully Added";
                } else {
                    $success = $this->payment_model->Update_leave_Info($id, $data);
                    echo "Successfully Updated";
                }
    ...
    public function Application()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['leavetypes']  = $this->payment_model->GetleavetypeInfo();
            $data['application'] = $this->payment_model->AllLeaveAPPlication();
            $this->load->view('backend/vendor_approve', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function EmApplication()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['leavetypes']  = $this->payment_model->GetleavetypeInfo();
            $data['application'] = $this->payment_model->GetallApplication($emid);
            $this->load->view('backend/vendor_apply', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Update_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $pur_no       = $this->input->post('pur_no');
            $ban_no       = $this->input->post('ban_no');
            $address      = $this->input->post('address');
            $invoice_no   = $this->input->post('invoice_no');
            $amount       = $this->input->post('amount');
            //$inv_recdate  = $this->input->post('inv_recdate');
            //$goods_recdate= $this->input->post('goods_recdate');
            $pay_duedate  = $this->input->post('pay_duedate');
            $des_payment  = $this->input->post('des_payment');
            $doc_prepared = $this->input->post('doc_prepared');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data    = array(
                    ...
                    'amount' => $amount,
                    //'inv_recdate' => $inv_recdate,
                    //'goods_recdate' => $goods_recdate,
                    'pay_duedate' => $pay_duedate,
                    'des_payment' => $des_payment,
                    'doc_prepared'=> $doc_prepared,
                    'ban_name' => $ban_name,
                ...
                $success = $this->payment_model->Application_Apply_Update($id, $data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                ...
                if ($this->db->affected_rows()) {
                    ...
                    $success = $this->payment_model->Application_Apply_Approve($data);
    ...
    public function Add_Applications()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (empty($id)) {
                    $success = $this->payment_model->Application_Apply($data);
                    #$this->session->set_flashdata('feedback','Successfully Updated');
                    #redirect("leave/Application");
                    echo "Successfully Added";
                } else {
    ...
    public function Add_L_Status()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $success = $this->payment_model->Application_Apply_Update($id, $data);
            if ($value == 'Approve') {
                $totalday = $this->payment_model->GetTotalDay($type);
                $total    = $totalday->total_day + $duration;
                $data     = array();
                $data     = array(
                    'total_day' => $total
                );
                $success  = $this->payment_model->Assign_Duration_Update($type, $data);
                echo "Successfully Updated";
            } else {
                echo "Successfully Updated";
            }
    ...
    public function Holidaybyib()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['holidayvalue'] = $this->payment_model->GetLeaveValue($id);
    ...
    public function LeaveAppbyid()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $data['leaveapplyvalue'] = $this->payment_model->GetLeaveApply($id);
            /*$leaveapplyvalue = $this->payment_model->GetEmLeaveApply($emid);*/
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function LeaveTypebYID()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id                     = $this->input->get('id');
            $data['leavetypevalue'] = $this->payment_model->GetLeaveType($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function GetEarneBalanceByEmCode()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id                     = $this->input->get('id');
            $data['earnval'] = $this->payment_model->GetEarneBalanceByEmCode($id);
            echo json_encode($data);
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function HOLIvalueDelet()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id      = $this->input->get('id');
            $success = $this->payment_model->DeletHoliday($id);
    ...
    public function APPvalueDelet()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id      = $this->input->get('id');
            $success = $this->payment_model->DeletApply($id);
            redirect('leave/Application');
        } else {
            redirect(base_url(), 'refresh');
        }
    ...
    public function LeavetypeDelet()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id      = $this->input->get('D');
            $success = $this->payment_model->DeletType($id);
            redirect('payment/paymenttypes');
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function paymenttypes()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $data['leavetypes'] = $this->payment_model->GetleavetypeInfo();
            $this->load->view('backend/paymenttypes', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function LeaveType()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $id          = $this->input->get('id');
            $year        = date('Y');
            $leavetype   = $this->payment_model->GetemLeaveType($id, $year);
            $assignleave = $this->payment_model->GetemassignLeaveType($id, $year);
    ...
    public function view(){
        ...
        $this->load->view('backend/my_cv/print_paymentapp',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}         
    ...
    public function EmLeavesheet()
    {
        $emid              = $this->session->userdata('user_login_id');
        $data              = array();
        $data['embalance'] = $this->payment_model->EmLeavesheet($emid);
    ...
    public function GetemployeeGmLeave()
    {
        ...
        $emid        = $this->input->get('emid');
        $assignleave = $this->payment_model->GetemassignLeaveType($emid, $id, $year);
        $totaldays   = 0;
        foreach ($assignleave as $value) {
            $totaldays = $totaldays + $value->day;
        }
        $day        = $totaldays;
        $leavetypes = $this->payment_model->GetleavetypeInfoid($id);
    ...
    public function Get_LeaveDetails()
    {
        ...
        $report = $this->payment_model->GetEmLEaveReport($emid, $day, $year);
    ...
    public function approveLeaveStatus() {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $success = $this->payment_model->updateAplicationAsResolved($id, $data);
            if ($value == 'Approve') {
                $determineIfNew = $this->payment_model->determineIfNewLeaveAssign($employeeId, $type);
                //How much taken
                $totalHour = $this->payment_model->getLeaveTypeTotal($employeeId, $type);
            ...
            $success  = $this->payment_model->updateLeaveAssignedInfo($employeeId, $type, $data);
            $earnval = $this->payment_model->emEarnselectByLeave($employeeId); 
              $data = array();
              $data = array(
                        'present_date' => $earnval->present_date - ($duration/8),
                        'hour' => $earnval->hour - $duration
                    );
            $success = $this->payment_model->UpdteEarnValue($employeeId,$data);                     
            echo "Updated successfully";
                ...
                //If not taken yet
                    ...
                    $success  = $this->payment_model->insertLeaveAssignedInfo($data);
                    echo "Updated successfully";
                }
            } else {
                echo "Updated successfully";
    ...
    public function LeaveAssign()
    {
        if ($this->session->userdata('user_login_access') != False) {
            ...
            if (!empty($leaveID)) {
                $year        = date('Y');
                $daysTaken = $this->payment_model->GetemassignLeaveType($employeeID, $leaveID, $year);
                //die($daysTaken->hour);
                $leavetypes = $this->payment_model->GetleavetypeInfoid($leaveID);
                ...
                if($leaveID =='5'){
                $earnTaken = $this->payment_model->emEarnselectByLeave($employeeID);
               ...
               /* $daysTaken = $this->payment_model->GetemassignLeaveType('Sah1804', 2, 2018);
                $leavetypes = $this->payment_model->GetleavetypeInfoid($leaveID);
    ...
    public function Earnedleave(){
       if ($this->session->userdata('user_login_access') != False) { 
           $data['employee']    = $this->employee_model->emselect();
            $data['earnleave'] = $this->payment_model->GetEarnedleaveBalance();
    ...
    public function Update_Earn_Leave(){
        ...
        $emid = $emcode->em_id;
        $earnval = $this->payment_model->emEarnselectByLeave($emid);
        if(!empty($earnval)){
              $data = array();
              $data = array(
        ...
        $success = $this->payment_model->UpdteEarnValue($emid,$data);          
        } else {
              $data = array();
              $data = array(
                        'em_id' => $emid,
        ...
        $success = $this->payment_model->Add_Earn_Leave($data);  
        }
