...
class Advance extends CI_Controller
{
	...
	public function View()
	{
		if ($this->session->userdata('user_login_access') != false) {
			...
			$data['loanview'] = $this->advance_model->loan_modeldata();
			$this->load->view('backend/advance', $data);
		} else {
	...
	public function Add_Loan()
	{
        ...
        $interestper = $this->input->post('interest')/100;*/
			...
			if ($this->form_validation->run() == false) {
				...
				#redirect("Advance/View");
			} else {
				#$emvalue = $this->advance_model->GetEmployeeForloancheck($em_id);
				#echo $emvalue->status;
				$data = array();
				$data = array(
					'emp_id' => $em_id,
				...
				if (empty($id)) {
					...
					if (!empty($emvalue->status)) {
						...
						$success = $this->advance_model->Add_LoanData($data);
						echo "Successfully Added";
					}
				} else {
					$success = $this->advance_model->update_LoanDataVal($id, $data);
	...
	public function installment()
	{
		if ($this->session->userdata('user_login_access') != false) {
			$data['employee'] = $this->employee_model->emselect();
			$data['installment'] = $this->advance_model->installmentSelect();
			$this->load->view('backend/advance_installment', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
	public function Add_Loan_Installment()
	{
		if ($this->session->userdata('user_login_access') != false) {
			...
			if ($this->form_validation->run() == false) {
				...
				if (empty($id)) {
					$loanvalue = $this->advance_model->GetLoanValuebyLId($loanid);
					...
					$success = $this->advance_model->Add_installData($data);
					...
					$success = $this->advance_model->update_LoanData($loanid, $data);
					echo "Successfully Added";
				} else {
					$data = array();
					$data = array(
					...
					$success = $this->advance_model->update_LoanInstallData($id, $data);
					echo "Successfully Updated";
				}
			}
		} else {
	...
	public function LoanByID()
	{
		if ($this->session->userdata('user_login_access') != false) {
			...
			$data['loanvalue'] = $this->advance_model->LoanValselect($id);
			$data['loanvalueem'] = $this->advance_model->LoanValEmselect($id);
			$data['loanvalueinstallment'] = $this->advance_model->LoanInstallValEmselect($id);
