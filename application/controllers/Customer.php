...
class Customer extends CI_Controller {
    ...
    public function Customers(){
        ...
        $data['customer'] = $this->customer_model->emselect();
	...
	$blood = $this->input->post('blood');		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->customer_model->Does_email_exists($email) && $password != $confirm){
            ...
            $success = $this->customer_model->Add($data);
	...
	$status = $this->input->post('status');		
        ...
        if ($this->form_validation->run() == FALSE) {
            echo validation_errors();
			#redirect("customer/view?I=" .base64_encode($eid));
			} else {
            if($_FILES['image_url']['name']){
            $file_name = $_FILES['image_url']['name'];
			$fileSize = $_FILES["image_url"]["size"]/1024;
            ...
            $employee = $this->customer_model->GetBasic($id);
    ...
    public function view(){
        ...
        $data['permanent'] = $this->customer_model->GetperAddress($id);
        $data['present'] = $this->customer_model->GetpreAddress($id);
        $data['education'] = $this->customer_model->GetEducation($id);
        //$data['experience'] = $this->customer_model->GetExperience($id);
        $data['bankinfo'] = $this->customer_model->GetBankInfo($id);
        $data['tradeinfo'] = $this->customer_model->GetTradeInfo($id);
        $data['auditinfo'] = $this->customer_model->GetAuditInfo($id);
        $data['fileinfo'] = $this->customer_model->GetFileInfo($id);
        $data['typevalue'] = $this->payroll_model->GetsalaryType();
        $data['leavetypes'] = $this->leave_model->GetleavetypeInfo();    
        $data['salaryvalue'] = $this->customer_model->GetsalaryValue($id);
        $data['socialmedia'] = $this->customer_model->GetSocialValue($id);
            $year = date('Y');
        $data['Leaveinfo'] = $this->customer_model->GetLeaveiNfo($id,$year);
    ...
    public function view1(){
        ...
        $this->load->view('backend/print_view',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}         
    ...
    public function Parmanent_Address(){
        ...
        if ($this->form_validation->run() == FALSE) {
            echo validation_errors();
			#redirect("customer/view?I=" .base64_encode($em_id));
			} else {
            $data = array();
                $data = array(
                    'emp_id' => $em_id,
            ...
            if(!empty($id)){
                $success = $this->customer_model->UpdateParmanent_Address($id,$data);
                $this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";                
            } else {
                $success = $this->customer_model->AddParmanent_Address($data);
    ...
    public function Add_Education(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_education($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_Education($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";
            }
    ...
    public function Save_Social(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Insert_Media($data);
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_Media($id,$data);
                echo "Successfully Updated";
            }
    ...
    public function Add_Experience(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_Experience($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                echo "Successfully Updated";
            } else {
                $success = $this->customer_model->Update_Experience($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                echo "Successfully Updated";
            }
    ...
    public function Disciplinary(){
        if($this->session->userdata('user_login_access') != False) {
        $data['desciplinary'] = $this->customer_model->desciplinaryfetch();
    ...
    public function add_Desciplinary(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_Desciplinary($data);
                $this->session->set_flashdata('feedback','Successfully Added');
                #redirect('customer/Disciplinary');
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_Desciplinary($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_bank_info(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_BankInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_BankInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_trade_info(){
     ...
     //$this->form_validation->set_rules('branch_name', 'branch name', 'trim|required|min_length[5]|max_length[120]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_TradeInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_TradeInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Add_audit_info(){
     ...
     //$this->form_validation->set_rules('branch_name', 'branch name', 'trim|required|min_length[5]|max_length[120]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
                $success = $this->customer_model->Add_AuditInfo($data);
                #$this->session->set_flashdata('feedback','Successfully Added');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Added";
            } else {
                $success = $this->customer_model->Update_AuditInfo($id,$data);
                #$this->session->set_flashdata('feedback','Successfully Updated');
                #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
            }
    ...
    public function Reset_Password_Hr(){
        ...
        $success = $this->customer_model->Reset_Password($id,$data);
        #$this->session->set_flashdata('feedback','Successfully Updated');
        #redirect("customer/view?I=" .base64_encode($id));
                echo "Successfully Updated";
            } else {
    ...
    public function Reset_Password(){
        ...
        $onep = $this->input->post('new1');
        $twop = $this->input->post('new2');
        $pass = $this->employee_model->GetCustomerId($id);
    ...
    public function Department(){
        if($this->session->userdata('user_login_access') != False) {
        $data['department'] = $this->customer_model->depselect();
    ...
    public function Save_dep(){
       ...
       if ($this->form_validation->run() == FALSE) {
           echo validation_errors();
           redirect('customer/Department');
       }else{
        $data = array();
        $data = array('dep_name' => $dep);
        $success = $this->customer_model->Add_Department($data);
        #$this->session->set_flashdata('feedback','Successfully Added');
        #redirect('customer/Department');
       }
        }
    ...
    $exp_date = $this->input->post('exp_date');  		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $success = $this->customer_model->File_Upload($data); 
            #$this->session->set_flashdata('feedback','Successfully Updated');
            #redirect("customer/view?I=" .base64_encode($em_id));
                echo "Successfully Updated";
			}
    ...
    public function Inactive_Customer(){
        $data['invalidem'] = $this->customer_model->getInvalidUser();
        $this->load->view('backend/invalid_customer',$data);
    }
}