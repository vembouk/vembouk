...
class Attendance_model extends CI_Model
{
    ...
    public function em_attendance()
    {
      ...
      FROM `attendance`
      LEFT JOIN `employee` ON `attendance`.`emp_id`=`employee`.`em_code` WHERE `attendance`.`status` = 'P' ORDER BY `attendance`.`id` DESC";
    ...
    public function getAttendanceDataByID($employee_id, $date_from, $date_to)
    {
      ...
      FROM `attendance`
      LEFT JOIN `employee` ON `attendance`.`emp_id` = `employee`.`em_code` 
      WHERE (`attendance`.`emp_id` = '$employee_id') AND (`atten_date` BETWEEN '$date_from' AND '$date_to') AND (`attendance`.`status` = 'P')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    ...
    {
      $sql = "SELECT TRUNCATE((SUM(ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )))/3600), 1) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employee_PIN') AND (`atten_date` BETWEEN '$date_from' AND '$date_to') AND (`attendance`.`status` = 'P')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
    public function getTotalAttendanceDataByID1($employee_PIN, $date_from, $date_to)
    {
      $sql = "SELECT TRUNCATE((SUM(ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time1`, `signout_time1` ) ) )))/3600), 1) AS Hours1 FROM `attendance` WHERE (`attendance`.`emp_id`='$employee_PIN') AND (`atten_date` BETWEEN '$date_from' AND '$date_to') AND (`attendance`.`status` = 'P')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
