<?php
    ...
    // get the vendor list from the vendors we add already
    ...
    public function Application_Apply($data){
        $this->db->insert('vendor_application',$data);
    }
    ...
    public function Application_Apply_Update($id, $data){
        $this->db->where('id', $id);
        $this->db->update('vendor_application', $data);         
    }
    ...
    public function GetLeaveType($id){
        $sql = "SELECT * FROM `payment_types` WHERE `type_id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
    public function GetleavetypeInfo(){
        $sql = "SELECT * FROM `payment_types` WHERE `status`='1' ORDER BY `type_id` DESC";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result; 
    }
    public function GetleavetypeInfoid($id){
        $sql = "SELECT * FROM `payment_types` WHERE `status`='1' AND `type_id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
    ...
    public function GetemLeaveType($id,$year){
        ...
        `payment_types`.`name`
        FROM `assign_leave`
        LEFT JOIN `payment_types` ON `assign_leave`.`type_id`=`payment_types`.`type_id`
    ...
    public function GetEmLEaveReport($emid, $day, $year){
        ...
        $sql = "SELECT `vendor_application`.*,
                (SELECT SUM(`leave_duration`) 
                    ...
                    `payment_types`.`name`
                FROM `vendor_application`
                    LEFT JOIN `payment_types` ON `vendor_application`.`typeid`=`payment_types`.`type_id`
                    LEFT JOIN `employee` ON `vendor_application`.`em_id`=`employee`.`em_id`
                WHERE MONTH(start_date) = '$day' AND YEAR(start_date) = '$year'";
    } else {

        $sql = "SELECT `vendor_application`.*, (SELECT SUM(`leave_duration`) 
       FROM vendor_application
       WHERE  `vendor_application`.`em_id` = '$emid' AND MONTH(start_date) = '$day' AND YEAR(start_date) = '$year') AS `total_duration`,
        `employee`.`first_name`,`last_name`,`em_code`, 
        `payment_types`.`name`
        FROM `vendor_application`
        LEFT JOIN `payment_types` ON `vendor_application`.`typeid`=`payment_types`.`type_id`
        LEFT JOIN `employee` ON `vendor_application`.`em_id`=`employee`.`em_id`
        WHERE `vendor_application`.`em_id` = '$emid' AND MONTH(start_date) = '$day' AND YEAR(start_date) = '$year'";
...
*/
    ...
    public function GetLeaveApply($id){
      ...
      FROM `vendor_application`
      LEFT JOIN `employee` ON `vendor_application`.`em_id`=`employee`.`em_id` 
      WHERE `vendor_application`.`id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
    ...
    $sql = "SELECT `vendor_application`.*,
      `employee`.`em_id`,`first_name`,`last_name`,`em_code`,
      `payment_types`.`type_id`,`name`
      FROM `vendor_application`
      LEFT JOIN `employee` ON `vendor_application`.`em_id`=`employee`.`em_id`
      LEFT JOIN `payment_types` ON `vendor_application`.`typeid`=`payment_types`.`type_id`
      WHERE `vendor_application`.`em_id`='$emid'";
        $query=$this->db->query($sql);
		$result = $query->result();
		return $result; 
    }
    ...
    $sql = "SELECT `vendor_application`.*,
      ...
      LEFT JOIN `employee` ON `vendor_application`.`em_id`=`employee`.`em_id`
      LEFT JOIN `payment_types` ON `vendor_application`.`typeid`=`payment_types`.`type_id`
      WHERE `vendor_application`.`leave_status`='Not Approve'";
        $query=$this->db->query($sql);
		$result = $query->result();
		return $result; 
    }
    ...
    public function DeletType($id){
        $this->db->delete('payment_types',array('type_id'=> $id));        
    }
    public function DeletApply($id){
        $this->db->delete('vendor_application',array('id'=> $id));        
    }
