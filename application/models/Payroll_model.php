...
public function getPinFromID($employeeID){
    ...
    {
      $sql = "SELECT TRUNCATE((SUM(ABS( `normal_hour` ) )), 1) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employeePIN') AND (`atten_date` BETWEEN '$start_date' AND '$end_date')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }

    public function totalHoursWorkedByEmployeeInAMonth1($employeePIN, $start_date, $end_date)
    {
      $sql = "SELECT TRUNCATE((SUM(ABS( `overtime` ) )), 1) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employeePIN') AND (`atten_date` BETWEEN '$start_date' AND '$end_date')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }

    public function totalHoursWorkedByEmployeeInAMonth2($employeePIN, $start_date, $end_date)
    {
      $sql = "SELECT TRUNCATE((SUM(ABS( `holiday_ot` ) )), 1) AS Hours FROM `attendance` WHERE (`attendance`.`emp_id`='$employeePIN') AND (`atten_date` BETWEEN '$start_date' AND '$end_date')";
        $query  = $this->db->query($sql);
        $result = $query->result();
        return $result;
    }
