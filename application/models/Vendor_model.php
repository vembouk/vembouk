<?php
  ...
  public function getvendor(){
    ...
    public function GetBasic($id){
      ...
      `department`.*
      FROM `vendor`
      LEFT JOIN `designation` ON `vendor`.`des_id`=`designation`.`id`
      LEFT JOIN `department` ON `vendor`.`dep_id`=`department`.`id`
      WHERE `em_id`='$id'";
        $query=$this->db->query($sql);
		$result = $query->row();
		return $result;          
    ...
    public function Projectvendor($id){
      ...
      `vendor`.`em_id`,`first_name`,`last_name`
      FROM `assign_task`
      LEFT JOIN `vendor` ON `assign_task`.`assign_user`=`vendor`.`em_id`
   ...
   /* public function GetExperience($id){
    ...
    public function desciplinaryfetch(){
      ...
      `vendor`.`em_id`,`first_name`,`last_name`,`em_code`
      FROM `desciplinary`
      LEFT JOIN `vendor` ON `desciplinary`.`em_id`=`vendor`.`em_id`";
        $query=$this->db->query($sql);
		$result = $query->result();
		return $result;        
    }
  ...
  public function Add_AuditInfo($data){
    ...
    public function GetvendorId($id){
        $sql = "SELECT `em_password` FROM `vendor` WHERE `em_id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
    public function GetFileInfo($id){
        $sql = "SELECT * FROM `vendor_file` WHERE `em_id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result; 
    }
    ...
    $this->db->insert('vendor_file',$data);
  }
